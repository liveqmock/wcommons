...
public class UrlUtilsTests {
	...
	@Test
	public void testSplitUrlParams2() {
		Assert.assertTrue("http://baidu.com/test!list.action?search=g.cn".matches(UrlUtils.STRUTS2_REGEX));
		Assert.assertTrue("/test.action?search=g.cn".matches(UrlUtils.STRUTS2_REGEX));
		Assert.assertTrue("/test.action".matches(UrlUtils.STRUTS2_REGEX));
		Assert.assertTrue("/test!list.action".matches(UrlUtils.STRUTS2_REGEX));
	}

	@Test
	public void testSplitUrlParams() {
		Assert.assertArrayEquals(UrlUtils.splitUrlParams(""), new String[] { "", "" });
		Assert.assertArrayEquals(UrlUtils.splitUrlParams(null), new String[] { "", "" });
		Assert.assertArrayEquals(UrlUtils.splitUrlParams("/test.do"), new String[] { "/test.do", "" });
		Assert.assertArrayEquals(UrlUtils.splitUrlParams("/test.do?action=list"), new String[] { "/test.do",
				"action=list" });

		Assert.assertArrayEquals(UrlUtils.splitUrlParams("/test.action"), new String[] { "/test.action", "" });
		Assert.assertArrayEquals(UrlUtils.splitUrlParams("/test.action?action=list"), new String[] { "/test.action",
				"action=list" });

		Assert.assertArrayEquals(UrlUtils.splitUrlParams("/test!list.action"), new String[] { "/test.action",
				"action=list" });
		Assert.assertArrayEquals(UrlUtils.splitUrlParams("/test!list.action?search=g.cn"), new String[] {
				"/test.action", "action=list&search=g.cn" });
	}
